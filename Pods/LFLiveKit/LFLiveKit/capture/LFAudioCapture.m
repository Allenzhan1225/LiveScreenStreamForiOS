...
static void * const kLFAudioSerialQueue = "com.youku.Laifeng.audioCapture.Queue";
...
- (instancetype)initWithAudioConfiguration:(LFLiveAudioConfiguration *)configuration{
    if(self = [super init]){
        ...
        self.isRunning = NO;
        self.taskQueue = dispatch_queue_create(kLFAudioSerialQueue, NULL);
        dispatch_queue_set_specific(self.taskQueue, kLFAudioSerialQueue, &kLFAudioSerialQueue, NULL);
...
- (void)dealloc {
    ...
    if (dispatch_get_specific(kLFAudioSerialQueue)) {
        deallocBlock();
    } else {
        dispatch_sync(self.taskQueue, deallocBlock);
    }
}
